...
3. Now first we check if the laplace transform of given function can be calculated by the code by checking if the limit tends to infinity of the function should be finite, if its not the code simply stops displaying the error message.
4. If the transform is possible our main challange is now to find a number big enough so that the integration value at infinity and at that number are almost the same, this is done in function upper bound.
...
**Note 2:** The need to input value of s: The final answer from comes out to be in addition of terms of trapezoidal rule(in form of y0+y1+y2......yn) without inputing s, which is very long.
